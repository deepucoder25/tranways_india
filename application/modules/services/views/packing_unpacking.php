<?php $this->load->view('template/inner_hero', ['page_title' => 'Packing & Unpacking', 'parent_title' => 'Services', 'parent_link' => 'services', 'bg_image' => 'assets/images/services/packing_unpacking_hero.png']); ?>

<!-- Main Content Area with Sidebar -->
<section class="hs-main-section py-5 hs-bg-pattern">
  <div class="container py-4">
    <div class="row g-5">
      
      <!-- Main Content Col (8) -->
      <div class="col-lg-8">
        
        <!-- Featured Image & Intro -->
        <div class="hs-card-premium bg-white rounded-4 shadow-sm p-4 p-md-5 mb-5 position-relative border-0">
          <div class="hs-premium-badge">Premium Care</div>
          <img src="<?= base_url('assets/images/services/packing_unpacking_hero.png') ?>" alt="<?= $company3 ?> Packing and Unpacking" class="img-fluid rounded-4 mb-4 w-100 object-fit-cover shadow" style="max-height: 450px;" loading="lazy">
          
          <h2 class="display-6 fw-bold text-dark mb-4">Professional <span class="text-primary-blue">Packing & Unpacking</span></h2>
          <p class="text-muted fs-5 mb-4 lh-lg">
            Packing is often the most time-consuming and stressful part of any move. Let our experts at <strong><?= $company3 ?></strong> take the burden off your shoulders. We use industry-best packaging materials including high-grade bubble wrap, customized corrugated boxes, and foam sheets to ensure your most fragile items are packed with the utmost care and security.
          </p>
          
          <div class="row g-4 mt-3">
            <div class="col-md-6">
              <div class="hs-feature-box d-flex align-items-center p-3 rounded-3 border">
                <div class="hs-feature-icon text-orange me-3">
                  <i class="bi bi-box-seam-fill fs-2"></i>
                </div>
                <div>
                  <h5 class="fw-bold mb-1">Premium Materials</h5>
                  <p class="text-muted small mb-0">3-layer protective wrapping.</p>
                </div>
              </div>
            </div>
            <div class="col-md-6">
              <div class="hs-feature-box d-flex align-items-center p-3 rounded-3 border">
                <div class="hs-feature-icon text-primary-blue me-3">
                  <i class="bi bi-tag-fill fs-2"></i>
                </div>
                <div>
                  <h5 class="fw-bold mb-1">Smart Labeling</h5>
                  <p class="text-muted small mb-0">Easy room-by-room unpacking.</p>
                </div>
              </div>
            </div>
          </div>
        </div>

        <!-- Inner Process Section -->
        <div class="hs-card-premium bg-white rounded-4 shadow-sm p-4 p-md-5 mb-5 border-0">
          <div class="d-flex align-items-center mb-4">
            <i class="bi bi-gear-fill text-orange fs-2 me-3"></i>
            <h3 class="fw-bold mb-0">Our proven <span class="text-primary-blue">4-Step</span> Process</h3>
          </div>
          
          <div class="row g-4 hs-process-grid">
            <div class="col-sm-6">
              <div class="hs-process-box p-4 rounded-4 bg-light h-100 position-relative overflow-hidden">
                <div class="hs-process-num">01</div>
                <h5 class="fw-bold text-dark position-relative z-index-2">Assessment</h5>
                <p class="text-muted small mb-0 position-relative z-index-2">We assess your items to determine the exact packing materials required.</p>
              </div>
            </div>
            <div class="col-sm-6">
              <div class="hs-process-box p-4 rounded-4 bg-light h-100 position-relative overflow-hidden">
                <div class="hs-process-num">02</div>
                <h5 class="fw-bold text-dark position-relative z-index-2">Careful Packing</h5>
                <p class="text-muted small mb-0 position-relative z-index-2">Fragile items receive multi-layer wrapping, while furniture is edge-protected.</p>
              </div>
            </div>
            <div class="col-sm-6">
              <div class="hs-process-box p-4 rounded-4 bg-light h-100 position-relative overflow-hidden">
                <div class="hs-process-num">03</div>
                <h5 class="fw-bold text-dark position-relative z-index-2">Inventory Logging</h5>
                <p class="text-muted small mb-0 position-relative z-index-2">Every box is inventoried, labeled, and tracked to prevent missing items.</p>
              </div>
            </div>
            <div class="col-sm-6">
              <div class="hs-process-box p-4 rounded-4 bg-light h-100 position-relative overflow-hidden">
                <div class="hs-process-num">04</div>
                <h5 class="fw-bold text-dark position-relative z-index-2">Unpacking Setup</h5>
                <p class="text-muted small mb-0 position-relative z-index-2">At the destination, we carefully unpack and arrange items as per your request.</p>
              </div>
            </div>
          </div>
        </div>

        <!-- Custom Inner Testimonials -->
        <div class="hs-card-premium bg-white rounded-4 shadow-sm p-4 p-md-5 mb-5 border-0">
          <div class="d-flex align-items-center justify-content-between mb-4 border-bottom pb-3">
            <h3 class="fw-bold mb-0"><i class="bi bi-star-fill text-warning me-2"></i> Client <span class="text-primary-blue">Stories</span></h3>
          </div>
          
          <div class="row g-4">
            <div class="col-md-6">
              <div class="hs-review-card bg-light p-4 rounded-4 h-100">
                <div class="d-flex text-warning mb-2">
                  <i class="bi bi-star-fill"></i><i class="bi bi-star-fill"></i><i class="bi bi-star-fill"></i><i class="bi bi-star-fill"></i><i class="bi bi-star-fill"></i>
                </div>
                <p class="fst-italic text-muted small mb-3">"The packing team was incredible. They wrapped my expensive antique vases so securely that I had absolutely zero anxiety during the move."</p>
                <div class="d-flex align-items-center">
                  <div class="hs-reviewer-avatar bg-primary-blue text-white rounded-circle d-flex align-items-center justify-content-center fw-bold me-3">A</div>
                  <div>
                    <h6 class="fw-bold mb-0">Aditi Varma</h6>
                    <span class="small text-muted">Homeowner</span>
                  </div>
                </div>
              </div>
            </div>
            <div class="col-md-6">
              <div class="hs-review-card bg-light p-4 rounded-4 h-100">
                <div class="d-flex text-warning mb-2">
                  <i class="bi bi-star-fill"></i><i class="bi bi-star-fill"></i><i class="bi bi-star-fill"></i><i class="bi bi-star-fill"></i><i class="bi bi-star-fill"></i>
                </div>
                <p class="fst-italic text-muted small mb-3">"Their unpacking service is a lifesaver. By the time they left, my kitchen and wardrobe were already perfectly organized."</p>
                <div class="d-flex align-items-center">
                  <div class="hs-reviewer-avatar bg-orange text-white rounded-circle d-flex align-items-center justify-content-center fw-bold me-3">K</div>
                  <div>
                    <h6 class="fw-bold mb-0">Karan Patel</h6>
                    <span class="small text-muted">Banker</span>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>

        <!-- Custom Inner FAQs -->
        <div class="hs-card-premium bg-white rounded-4 shadow-sm p-4 p-md-5 mb-5 border-0">
           <div class="d-flex align-items-center mb-4 border-bottom pb-3">
             <i class="bi bi-question-circle-fill text-orange fs-2 me-3"></i>
            <h3 class="fw-bold mb-0">Frequently Asked <span class="text-primary-blue">Questions</span></h3>
          </div>
          
          <div class="accordion hs-accordion" id="hsFaqAccordion">
            <div class="accordion-item border-0 mb-3 rounded-3 shadow-sm overflow-hidden">
              <h2 class="accordion-header">
                <button class="accordion-button fw-bold bg-white text-dark" type="button" data-bs-toggle="collapse" data-bs-target="#hsFaq1">
                  What kind of packing materials do you use?
                </button>
              </h2>
              <div id="hsFaq1" class="accordion-collapse collapse show" data-bs-parent="#hsFaqAccordion">
                <div class="accordion-body text-muted small">
                  We use top-tier materials including high-grade corrugated boxes, heavy-duty bubble wrap, stretch film, thermocol, and foam edge protectors to guarantee maximum safety.
                </div>
              </div>
            </div>
            
            <div class="accordion-item border-0 mb-3 rounded-3 shadow-sm overflow-hidden">
              <h2 class="accordion-header">
                <button class="accordion-button collapsed fw-bold bg-white text-dark" type="button" data-bs-toggle="collapse" data-bs-target="#hsFaq2">
                  Do you unpack everything at the destination?
                </button>
              </h2>
              <div id="hsFaq2" class="accordion-collapse collapse" data-bs-parent="#hsFaqAccordion">
                <div class="accordion-body text-muted small">
                  Yes! Our full-service package includes completely unpacking your boxes and placing items (like utensils, clothes, and books) into their designated shelves and cupboards.
                </div>
              </div>
            </div>
          </div>
        </div>

      </div>

      <!-- Sidebar Col (4) -->
      <div class="col-lg-4">
        <div class="position-sticky hs-sidebar-wrapper" style="top: 100px;">
          
          <div class="hs-sidebar-widget bg-white rounded-4 shadow-sm p-4 mb-4 border-top border-4 border-primary-blue">
            <h4 class="fw-bold mb-4">Our <span class="text-primary-blue">Guarantees</span></h4>
            <ul class="list-unstyled mb-0">
              <li class="d-flex align-items-center mb-3 pb-3 border-bottom">
                <i class="bi bi-check-circle-fill text-success fs-4 me-3"></i>
                <div>
                  <h6 class="fw-bold mb-0">No Hidden Costs</h6>
                  <span class="text-muted small">Transparent pricing policy.</span>
                </div>
              </li>
              <li class="d-flex align-items-center mb-3 pb-3 border-bottom">
                <i class="bi bi-clock-history text-orange fs-4 me-3"></i>
                <div>
                  <h6 class="fw-bold mb-0">On-Time Delivery</h6>
                  <span class="text-muted small">We respect your schedule.</span>
                </div>
              </li>
              <li class="d-flex align-items-center mb-3 pb-3 border-bottom">
                <i class="bi bi-box2-heart text-danger fs-4 me-3"></i>
                <div>
                  <h6 class="fw-bold mb-0">Zero Damage</h6>
                  <span class="text-muted small">Premium packing materials.</span>
                </div>
              </li>
              <li class="d-flex align-items-center">
                <i class="bi bi-person-badge text-primary-blue fs-4 me-3"></i>
                <div>
                  <h6 class="fw-bold mb-0">Verified Staff</h6>
                  <span class="text-muted small">Background-checked crew.</span>
                </div>
              </li>
            </ul>
          </div>

          <!-- Other Services List -->
          <div class="hs-sidebar-widget bg-white rounded-4 shadow-sm p-4 mb-4 border border-light">
            <h4 class="fw-bold mb-4 border-bottom pb-2">More Services</h4>
            <ul class="list-unstyled hs-services-list mb-0">
              <li><a href="<?= site_url('services/household-shifting') ?>">Household Shifting <i class="bi bi-chevron-right"></i></a></li>
              <li><a href="<?= site_url('services/office-relocation') ?>">Office Relocation <i class="bi bi-chevron-right"></i></a></li>
              <li><a href="<?= site_url('services/vehicle-transport') ?>">Vehicle Transport <i class="bi bi-chevron-right"></i></a></li>
              <li><a href="<?= site_url('services/packing-unpacking') ?>" class="active">Packing & Unpacking <i class="bi bi-chevron-right"></i></a></li>
              <li><a href="<?= site_url('services/loading-unloading') ?>">Loading & Unloading <i class="bi bi-chevron-right"></i></a></li>
              <li><a href="<?= site_url('services/local-shifting') ?>">Local Shifting <i class="bi bi-chevron-right"></i></a></li>
              <li><a href="<?= site_url('services/domestic-shifting') ?>">Domestic Shifting <i class="bi bi-chevron-right"></i></a></li>
              <li><a href="<?= site_url('services/storage-solutions') ?>">Storage Solutions <i class="bi bi-chevron-right"></i></a></li>
            </ul>
          </div>

          <!-- Help / Contact Card -->
          <div class="hs-sidebar-widget hs-help-card bg-orange text-white rounded-4 shadow p-4 py-4 text-center position-relative overflow-hidden">
            <div class="hs-help-pattern position-absolute opacity-25">
              <i class="bi bi-headset display-1"></i>
            </div>
            <div class="position-relative z-index-2">
              <div class="hs-help-icon mb-3 bg-white text-orange rounded-circle d-inline-flex p-3 shadow position-relative">
                <i class="bi bi-telephone-outbound fs-3 hs-animated-ring"></i>
              </div>
              <h4 class="fw-bold mb-2">Ready to Relocate?</h4>
              <p class="small text-white-75 mb-3 px-2">Call us directly for an instant moving estimate.</p>
              <a href="<?= $phonehtml ?>" class="text-white text-decoration-none fs-4 fw-extrabold d-block mb-4 hs-phone-link">
                <?= $phone ?>
              </a>
              <a href="<?= site_url('contacts') ?>" class="btn btn-light rounded-pill px-4 py-2 fw-bold text-orange w-100 shadow-sm hs-btn-pulse">Contact Us Now</a>
            </div>
          </div>

        </div>
      </div>

    </div>
  </div>
</section>
